...
class ContentBlockSerializer(serializers.ModelSerializer):
    """Сериализатор для редактируемых блоков контента"""
    class Meta:
        model = ContentBlock
        fields = ['id', 'page_key', 'block_key', 'content', 'content_type', 'updated_by', 'updated_at', 'created_at']
        read_only_fields = ['id', 'updated_by', 'updated_at', 'created_at']
        # Отключаем UniqueTogetherValidator, чтобы не падать с non_field_errors при идемпотентном создании
        validators = []
...
class GalleryImageSerializer(serializers.ModelSerializer):
    class Meta:
        model = GalleryImage
        fields = ['id', 'page_key', 'image', 'caption', 'order', 'uploaded_by', 'created_at']
